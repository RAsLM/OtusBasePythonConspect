...
async def do_some_db_query_async(to_sleep=0.1):
    ...
    # sleep(.1) not work with async code
def foo_sync():
...
async def get_users_profile_pic(user_id):
    log.info("start fetching user_profile_pic")
    await asyncio.sleep(.1)
    log.info("finish fetching user_profile_pic")
    return b"acd"


async def get_users_close_friends(user_id):
    log.info("start fetching get_users_close_friends")
    await asyncio.sleep(.2)
    log.info("finish fetching get_users_close_friends")
    return {"user_id": user_id, "friends": [{"id": 1, "name": "John"}, {"id": 1, "name": "Ann"}]}
...
async def load_users_data():
    ...
    log.info("close_pic %s", close_friends)
